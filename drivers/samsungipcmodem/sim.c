				...
				ofono_query_facility_lock_cb_t cb, void *data)
{
	...
	if(ipc_device_enqueue_message(sd->device, IPC_SEC_PHONE_LOCK, IPC_TYPE_GET,
						req, sizeof(struct ipc_sec_phone_lock_request_get_data),
				...
				ofono_sim_lock_unlock_cb_t cb, void *data)
{
	...
	if(ipc_device_enqueue_message(sd->device, IPC_SEC_CHANGE_LOCKING_PW, IPC_TYPE_SET,
						req, sizeof(struct ipc_sec_change_locking_pw_data),
				...
				ofono_sim_lock_unlock_cb_t cb, void *data)
{
	...
	ipc_sec_phone_lock_request_set_setup(req, pin_type, enable, (char*) passwd);

	if(ipc_device_enqueue_message(sd->device, IPC_SEC_PHONE_LOCK, IPC_TYPE_SET,
						req, sizeof(struct ipc_sec_phone_lock_request_set_data),
						pin_enable_cb, cbd) > 0)
		return;
				...
				ofono_sim_lock_unlock_cb_t cb, void *data)
{
	...
	ipc_sec_pin_status_setup(req, IPC_SEC_PIN_TYPE_PIN1, (char*) passwd, (char*) puk);

	if(ipc_device_enqueue_message(sd->device, IPC_SEC_PIN_STATUS, IPC_TYPE_SET,
						req, sizeof(struct ipc_sec_pin_status_request_data),
						puk_send_cb, cbd) > 0)
		return;
...
static void read_binary_cb(uint16_t cmd, void *data, uint16_t length, uint8_t error, void *user_data)
{
	...
	DBG("rsim_access_cb: sw1=%02x, sw2=%02x, len=%i", resp->sw1, resp->sw2, resp->length);
	...
	response = (const guint8*) data + sizeof(struct ipc_sec_rsim_access_response_header);
					...
					ofono_sim_read_cb_t cb, void *cb_data)
{
	...
	if(ipc_device_enqueue_message(sd->device, IPC_SEC_RSIM_ACCESS, IPC_TYPE_GET,
						req, sizeof(struct ipc_sec_rsim_access_request_header),
					...
					ofono_sim_passwd_cb_t cb, void *user_data)
{
	...
	if(ipc_device_enqueue_message(sd->device, IPC_SEC_PIN_STATUS, IPC_TYPE_GET,
...
static void pin_send_cb(uint16_t cmd, void *data, uint16_t length, uint8_t error, void *user_data)
{
	...
	sd->sim_state_query = ipc_device_add_notifcation_watch(sd->device,
												IPC_SEC_PIN_STATUS, notify_sim_status_cb, cbd);
}
			...
			ofono_sim_lock_unlock_cb_t cb, void *data)
{
	...
	ipc_sec_pin_status_setup(req, IPC_SEC_PIN_TYPE_PIN1, (char*) passwd, NULL);
...
static void lock_info_cb(uint16_t cmd, void *data, uint16_t length, uint8_t error, void *user_data)
{
	...
	case IPC_SEC_PIN_TYPE_PIN1:
		sd->pin_attempts[OFONO_SIM_PASSWORD_SIM_PIN] = resp->retry_count;
		...
		if(ipc_device_enqueue_message(sd->device, IPC_SEC_LOCK_INFOMATION, IPC_TYPE_GET,
						req, sizeof(struct ipc_sec_lock_infomation_request_data),
						lock_info_cb, cbd) > 0)
			return;

		goto error;
	case IPC_SEC_PIN_TYPE_PIN2:
		sd->pin_attempts[OFONO_SIM_PASSWORD_SIM_PIN2] = resp->retry_count;
		break;
	}
				...
				void *user_data)
{
	...
	ipc_device_enqueue_message(sd->device, IPC_SEC_PIN_STATUS, IPC_TYPE_GET,
